...
def create_assessment():
    ...
    if not document_id:
        return jsonify({"error": "请选择一个文档"}), 400
    ...
    if not doc_content.strip():
        return jsonify({"error": "文档内容为空，无法生成题目"}), 400

    # 并发执行出题 + 知识点提取
    _assessment_status[f"create_{document_id}"] = {"stage": "generating", "stage_label": "正在生成题目..."}

    with ThreadPoolExecutor(max_workers=2) as pool:
        future_questions = pool.submit(llm.generate_assessment, doc_content, question_count)
        ...
        assessment_id = AssessmentDAO.create(document_id, user_id=g.user_id)
        ...
        for kp in kp_list:
            KnowledgeDAO.upsert(document_id, kp.get("name", ""), mastery="learning", is_correct=None, user_id=g.user_id)

    _assessment_status.pop(f"create_{document_id}", None)
...
def get_assessment(assessment_id):
    ...
    return jsonify({"assessment": assessment, "questions": display_questions})
...
def submit_assessment(assessment_id):
    ...
    for r in results:
        ...
        if idx < len(questions):
            ...
            if kp_name:
                KnowledgeDAO.upsert(
                    assessment["document_id"], kp_name, mastery,
    ...
    if avg_score >= 80:
        ProgressDAO.update(doc_id, status="completed", confidence=avg_score)
    else:
        ProgressDAO.update(doc_id, status="review_needed", confidence=avg_score)

    conn = get_db()
    existing = conn.execute(
        "SELECT notes FROM learning_progress WHERE document_id=?", (doc_id,)
    ).fetchone()
    ...
    new_entry = f"\n## {timestamp} 测评 {final_score}分 ({correct_count}/{total})\n薄弱点：{weak_text}\n"
    ...
    StudySessionDAO.create(doc_id, session_type="practice", questions=total, user_id=g.user_id)
...
def assessment_history():
    ...
    if doc_id:
        rows = conn.execute(
            ...
            "WHERE a.document_id = ? AND a.status = 'completed' AND a.user_id = ? "
            "ORDER BY a.created_at DESC LIMIT ?",
            (doc_id, g.user_id, limit)
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT a.*, d.filename FROM assessments a "
